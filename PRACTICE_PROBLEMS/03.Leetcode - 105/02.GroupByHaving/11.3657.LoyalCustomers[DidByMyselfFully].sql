...
calculated as the number of refund transactions divided by the total number of transactions (purchases plus refunds).
...
Just one part regarding date difference I was stuck: DATEDIFF(last_date, first_date) --> returns difference in DAYS
I also used COUNT(condition) instead of SUM(condition)

*/

WITH tableWithFirstAndLastDateAdded AS (
...
FROM (
    SELECT
        ...
        DATEDIFF(last_date, first_date) AS days_active,
        SUM(transaction_type = "purchase") AS purchase_transactions,
        (100 * (SUM(transaction_type = "refund") / COUNT(*))) AS refund_rate
    FROM tableWithFirstAndLastDateAdded
