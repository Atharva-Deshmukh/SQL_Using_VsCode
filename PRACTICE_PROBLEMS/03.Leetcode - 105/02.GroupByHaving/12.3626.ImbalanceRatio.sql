...
I could think of logic but could not code it.
I could think of CTE

But using ROW_NUMBER() to number rows with max and min price values I couldn't think
Also, conditional aggregate, when we need only one value per group needed, I couldn't think
...
collapsed AS (
    SELECT
        ...
        -- Conditional Aggregate concept
        -- MAX() = used as a way to pick the one non-NULL value after the CASE
...
WHERE product_count >= 3
  AND expensive_quantity < cheapest_quantity
