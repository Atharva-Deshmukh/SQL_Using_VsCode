...
I made silly mistake here, everything else was fully correct

*/

WITH arrangeAndClassifyFirstOrder AS (
    SELECT delivery_id,
           customer_id,
           order_date,
...
filterFirstOrders AS (
    SELECT delivery_id,
           customer_id,
           orderType,
           rnk
    FROM arrangeAndClassifyFirstOrder 
    WHERE rnk = 1
)

SELECT ROUND(
   100 * (SELECT COUNT(*) FROM filterFirstOrders WHERE orderType = 'immediate')  / (SELECT COUNT(*) FROM filterFirstOrders)
    , 2
