...
I could think of 60 % of logic, but struggled to code it
*/
...
yearWiseHalvesTable AS (
    SELECT
        driver_id,
        driver_name,
        distance_km / fuel_consumed AS efficiency,
        CASE
            WHEN MONTH(trip_date) <= 6 THEN 'FirstHalf'
            ELSE 'SecondHalf'
        END AS halfType
    FROM joinedTables
),
...
driverAverages AS (
    SELECT
        ...
        AVG(
            CASE
                WHEN halfType = 'FirstHalf' THEN efficiency
            END
        ) AS first_half_avg,

        AVG(
            CASE
                WHEN halfType = 'SecondHalf' THEN efficiency
            END
        ) AS second_half_avg

    FROM yearWiseHalvesTable
    GROUP BY driver_id, driver_name

    -- Driver must have trips in BOTH halves
    HAVING
        COUNT(CASE WHEN halfType = 'FirstHalf' THEN 1 END) > 0
        AND
        COUNT(CASE WHEN halfType = 'SecondHalf' THEN 1 END) > 0
)
...
-- "Improved" means second half is better
