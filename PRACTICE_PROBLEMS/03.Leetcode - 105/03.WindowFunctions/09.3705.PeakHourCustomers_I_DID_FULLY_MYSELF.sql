...
I was just stuck at extraction of time and validating its range

We have this in MySQL: TIME(order_timestamp) BETWEEN '11:00:00' AND '14:00:00'

*/

-- do all necessary calculations first
WITH calculationsTable AS (
    SELECT customer_id,
           ROUND(AVG(order_rating), 2) AS average_rating, -- excludes null ratings
           COUNT(order_id) AS total_orders,
           COUNT(order_rating) AS totalRatedOrders, -- Excludes null ratings,
           SUM(
                TIME(order_timestamp) BETWEEN '11:00:00' AND '14:00:00'
                OR
                TIME(order_timestamp) BETWEEN '18:00:00' AND '21:00:00'
               ) AS noOfOrdersDuringPeakHours
...
SELECT customer_id,
       total_orders,
       ROUND((noOfOrdersDuringPeakHours/total_orders) * 100, 0) AS peak_hour_percentage,
       average_rating
FROM calculationsTable
WHERE total_orders >= 3
  AND average_rating >= 4.00
  AND (totalRatedOrders/total_orders) * 100 > 50
  AND  (noOfOrdersDuringPeakHours/total_orders) * 100 >= 60
